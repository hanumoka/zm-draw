...
**Phase**: Phase 7 완료 ✅ → **Phase 8 준비 중**
**목표 변경**: ~~Figma 다이어그램 에디터~~ → **FigJam 스타일 실시간 협업 화이트보드**
**진행률**: 다이어그램 에디터 Phase 1-7 완료 / 협업 화이트보드 Phase 8-17 예정

### 방향 전환 결정 (2026-01-28)

FigJam 기능 분석 결과, 다음 핵심 기능 추가 필요:
1. **스티키 노트** - 화이트보드 핵심 요소
2. **펜/마커 도구** - 자유로운 표현
3. **Yjs 실시간 협업** - 커서 공유, 동시 편집
4. **투표/타이머/스탬프** - 협업 경험 향상
...
- **미니맵 구현** ✅ NEW
  - Minimap 컴포넌트 생성 (Canvas 2D로 렌더링)
  - 도형/커넥터 축소 표시
  - 현재 뷰포트 위치 표시 (파란 사각형)
  - 미니맵 클릭/드래그로 뷰포트 이동
  - 캔버스 오른쪽 하단에 배치
...
- **Phase 4: 레이어 패널 완료** ✅
  ...
  - **잠금 토글** (🔒): locked shape은 선택/드래그 불가
  - **레이어 이름 편집**: 더블클릭으로 인라인 편집
  - **레이어 순서 변경**: HTML5 Drag API로 드래그앤드롭
  - DrawCanvasHandle에 setShapes() 메서드 추가
  - 레이어 클릭 시 캔버스 선택 동기화
...
## 다음 작업: Phase 8 (FigJam 스타일 화이트보드)

### Phase 8: 스티키 노트 + 펜 도구 (1주 예상)
